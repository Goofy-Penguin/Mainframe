...
namespace mainframe::networking {
	...
	template<typename T>
	concept isNetworkReadable = requires(T t, Packet p) {
		{ t.networkRead(p) };
	};

	template<typename T>
	concept isNetworkWritable = requires(T t, Packet p) {
		{ t.networkWrite(p) };
	};
	...
	public:
		...
		void read(std::optional<T>& ret) {
			if(read<bool>()) ret = read<T>();
			else ret = std::nullopt;
		}
